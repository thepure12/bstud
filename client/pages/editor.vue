<template>
    <div>
        <b-row class="mb-3 py-2 justify-content-around sticky-top bg-dark text-white">
            ...
            <b-row class="tools my-auto gap-2">
                <div v-for="tool in tools" :key="tool.name" :id="`tool-${tool.name}`" class="tool"
                    ...
                    <b-popover v-if="activeTool === tool" :target="`tool-${tool.name}`" placement="bottom"
                        triggers="click blur">
                        <verte :id="`${tool.name}-picker`" display="widget" :value="tool.color" @input="onColorChange">
                        </verte>
